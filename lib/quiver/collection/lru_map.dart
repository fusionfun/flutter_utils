...
import 'package:flutter_utils/quiver/iterables.dart' show GeneratingIterable;
import 'package:flutter_utils/quiver/cache/cache.dart';
...
abstract class LruMap<K, V> implements Map<K, V> {
  ...
  factory LruMap({int? maximumSize, RemoveCallback<V>? onUnsolicitedRemove}) =
...
class LinkedLruHashMap<K, V> implements LruMap<K, V> {
  RemoveCallback<V>? onUnsolicitedRemove;

  /// Create a new LinkedLruHashMap with a [maximumSize].
  factory LinkedLruHashMap({int? maximumSize, RemoveCallback<V>? onUnsolicitedRemove}) =>
      LinkedLruHashMap._fromMap(HashMap<K, _LinkedEntry<K, V>>(), maximumSize: maximumSize);

  LinkedLruHashMap._fromMap(this._entries, {int? maximumSize, this.onUnsolicitedRemove})
  ...
  V putIfAbsent(K key, V ifAbsent()) {
    final entry = _entries.putIfAbsent(key, () => _createEntry(key, ifAbsent()));
  ...
  void _removeLru() {
    ...
    if (entry != null) {
      onUnsolicitedRemove?.call(entry.value);
    }
