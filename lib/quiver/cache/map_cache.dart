...
import 'package:flutter_utils/quiver/collection/lru_map.dart' show LruMap;
...
class MapCache<K, V> implements Cache<K, V> {
  ...
  factory MapCache.lru({int? maximumSize, RemoveCallback<V>? onUnsolicitedRemove}) {
    // TODO(cbracken): inline the default value here for readability.
    // https://github.com/google/quiver-dart/issues/653
    return MapCache<K, V>(map: LruMap(maximumSize: maximumSize, onUnsolicitedRemove: onUnsolicitedRemove));
  }
  ...
  Future<void> addAll(Map<K, V> map) async {
    _map.addAll(map);
    return;
  }
  ...
  @override
  void clear(RemoveCallback<V> onRemove) {
    for (var data in _map.values) {
      onRemove.call(data);
    }
    _map.clear();
  }
